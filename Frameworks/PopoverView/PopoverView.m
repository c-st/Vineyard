...
#define kGradientBottomColor [UIColor cellarBeigeColour]  //[UIColor colorWithRed:0.98f green:0.98f blue:0.98f alpha:kBoxAlpha]
...
#define kGradientTitleBottomColor [[UIColor cellarWineRedColour] colorWithAlphaComponent:0.7] //[UIColor colorWithRed:0.93f green:0.93f blue:0.93f alpha:kBoxAlpha]
...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withText:(NSString *)text delegate:(id<PopoverViewDelegate>)delegate {
    ...
    popoverView.popoverDelegate = delegate;
    return popoverView;
}
...
- (void)tapped:(UITapGestureRecognizer *)tap {
    ...
    for(int i = 0; i < subviewsArray.count && !found; i++) {
        ...
        if(CGRectContainsPoint(view.frame, point)) {
            ...
            if(popoverDelegate && [popoverDelegate respondsToSelector:@selector(popoverView:didSelectItemAtIndex:)]) {
                [popoverDelegate popoverView:self didSelectItemAtIndex:i];
            }
...
- (void)didTapButton:(UIButton *)sender {
    ...
    if(popoverDelegate && [popoverDelegate respondsToSelector:@selector(popoverView:didSelectItemAtIndex:)]) {
        [popoverDelegate popoverView:self didSelectItemAtIndex:index];
    }
}
...
- (void)dismiss {
    [UIView animateWithDuration:0.3f animations:^{
        ...
        if(self.popoverDelegate && [self.popoverDelegate respondsToSelector:@selector(popoverViewDidDismiss:)]) {
            [popoverDelegate popoverViewDidDismiss:self];
        }
    }];
}
